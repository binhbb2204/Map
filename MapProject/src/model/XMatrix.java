...
public class XMatrix extends Matrix<XNode>{
        ...
        for(int row = 0; row < rows; row++){
            for(int col = 0; col < cols; col++){
                ...
                if(nodeListener != null){
                    node.addPropertyChangeListener(AppConstant.NodePredecessorChanged, nodeListener);
                }
            }
        }
    }
    ...
    private void buildEdges(int row, int col){
        ...
        for(int i = -1; i <= 1; i++){
            ...
            for(int j = -1; j <= 1; j++){
                int x = col + j;
                if(x < 0 || x >= cols){ // outrange
                    continue;
                }
                if (y == row && x == col) { // it is the node itself
					continue;
				}

                if (i == -1 && j == -1) { // no cross cut
